...
Released `0.1.0`: `package.json` and `extension/manifest.json` bumped from `0.0.0`, the changelog's `[Unreleased]` section dated, the README's install route switched back to npm with an npm badge, and the package published by hand (npm has no way to attach a trusted publisher to a package that does not exist yet). Two things worth remembering: `npm publish` from a non-interactive shell does not wait on npm's browser 2FA flow, it prints the auth URL and exits `EOTP` straight away, so the first publish needs a real terminal or `--otp`; and `npm view` answered `E404` for about a minute after the package was live, so a 404 there is not evidence the publish failed. The `v0.1.0` tag was pushed afterwards and the Release workflow went green by skipping its publish step, which is what the version check is for. The GitHub Actions trusted publisher was configured on npmjs.com the same day; `0.1.1` is the first release that will actually exercise it.
